...
namespace Multigroup.Portal.Controllers
{
    ...
    {
        public const string ErrorLoginMessage = "Your login has failed.";
        ...
        {
            if (_profileProvider.Authenticated)
            {
                var user = _profileProvider.CurrentUserId;
                var usuario = _userService.GetUser(user);
                if (!usuario.Active)
                {
                    ViewBag.ErrorMessage = "Login Incorrecto";
                    return PartialView();
                }
                else
                {
                    _userService.AddUserLogin(usuario.UserName, usuario.Names + " " + usuario.Surname);
        ...
        public ActionResult Login(LoginViewModel model)
        {
            ...
            {
                ViewBag.ErrorMessage = ErrorLoginMessage;

                return View("Index");
            }

            var isLogged = _userLoginProvider.Login(model.UserName, model.Password);
